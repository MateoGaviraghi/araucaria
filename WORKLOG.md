...
**Updated:** 2026-09-23, calendario y formulario de consulta en producción y verificados ahí. Mateo los miró ("está todo correcto"). Falta que nombre la próxima sección; el rediseño del panel `/admin` está pedido para después.

| | |
|---|---|
| **Current task** | Phase 1 of the Araucaria site. Live: WU-01 … WU-06, the `HERO`, the gallery (block 3, `D-028` → `D-031`) and now **the availability calendar with the inquiry form** (block 6, `D-033` + `D-034`), one section right after the gallery, `id="disponibilidad"`. Still to build on the landing: `NAV` (block 1), `FEATURE-GRID` (4), `PRICING-TABLE` (5), `MAP-LOCATION` (7), `FOOTER` (8) — Mateo names the order |
| **Real status** | `main` = `origin/main` = `e483be2` (pushed 2026-09-23; Vercel served it ~50 s later). The branch `wu-08-calendario-y-formulario` was fast-forwarded into `main` and can be deleted. **Verified in production** (`araucaria-multiespacio.vercel.app`): the calendar entrance at 1440 and 375 (numbers rise, lights on, zero overflow, zero console errors); the form end to end ("Otro (Bautismo)", 35 personas, phone "03425162081" → "+54 3425162081") opens `wa.me` **with a number** and the exact message of `05-API-CONTRACTS.md` §3. The production database has no taken days, so only free days show there. **The WhatsApp number in Vercel is Mateo's test number** until the client sends `CI-01`: real inquiries would reach Mateo. **Uncommitted:** this `WORKLOG.md` and `docs/10-MEMORY.md` (the `/cerrar` record) — Mateo did not ask for a commit. `.claude/launch.json` is committed (`OQ-08` closed); production still has no admin credential, on purpose until WU-09 |
| **Last chat** | 2026-09-23 (began with `/retomar`) |
| **Waiting on me (Claude)** | Nothing |
| **Waiting on Mateo** | (a) Whether to commit the `/cerrar` record (and push it). (b) Look at the calendar and form on a real phone (he checked them on his screen: "está todo correcto"). (c) Name the next section. (d) **The redesign of the owner panel `/admin`**, asked for "luego": *"quiero que modifiquemos todo el dashboard admin porque no me gustó nada, y que implementemos /diseno, /web-distintiva y los mejores criterios para dashboard de `C:\\Users\\mateo\\Downloads\\CRITERIO-DISENO.md`, usando animación de alertas, GSAP, etc."* — a full `seccion-premium` round when he calls it. (e) Whether the branch `wu-08-calendario-y-formulario` can be deleted. (f) He holds the dev passphrase (he rotated it this session with `npm run rotate-password`); it lives only in his password manager |
| **Waiting on third parties** | The client owes `CI-01` (WhatsApp number — until then production uses Mateo's test number), `CI-02`, `CI-03`, `CI-05` (VAT and price validity, still `{{CONFIRMAR}}`) and `CR-01`…`CR-03` (original photos, videos and logo). All colours stay `{{CONFIRMAR}}` until the logo arrives. **If `CR-02` arrives filmed wide, `D-027` … `D-030` reopen** |
| **Next action** | On `/retomar`: say the `/cerrar` record is uncommitted and ask whether to commit (and push) it. Then wait for Mateo: phone corrections to the calendar/form, the next section, or the `/admin` redesign — each one starts `seccion-premium` from its first step (references before plans) |
| **Do not touch** | Pushing `main` without Mateo's word · `.env.local` — **not even to count or grep a variable** (Mateo stopped a command that did) · the Vercel environment variables · the production Neon branch (five tables, no credential; a migration there is class `R3`) · the two real blocks in the `dev` branch (2026-09-19 Mediodía, 2026-09-20 Noche) · the Neon branch `backup-pre-0000-init` · **the designs already rejected**: `D-023`, `D-025`, `D-027`, `D-029`, `D-030` (see those entries), `D-033` (the hatched half-day, three columns, the photo behind a card, flat booking-site markers) and `D-034` ("Tarjeta con canto", the 35 lit seats, the three reference rounds, reference colours instead of the brand's) |
