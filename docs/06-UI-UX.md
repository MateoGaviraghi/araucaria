...
|---|---|---|---|
| 1 | `NAV.sections` | Brand + one always-reachable way around | Built 2026-09-25 (`D-040`, replaces the "sacarlo" of `D-036`). At the very top, over the hero: logo + links (El espacio · Qué incluye · Disponibilidad · Dónde estamos), no quick CTA. Below 160 px of scroll a round fixed button (top right) opens a right-hand panel with a curved edge: the same links large, "Consultar disponibilidad", WhatsApp and Instagram. Phone: the button from the start; it hides while scrolling down and returns on scrolling up |
