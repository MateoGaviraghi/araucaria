...
|---|---|---|---|
| 1 | `NAV.sections` | Brand + one always-reachable way around | Built 2026-09-25 (`D-041`, replaces the round button of `D-040`). The whole header is fixed at the top of every section: logo + links (El espacio · Qué incluye · Disponibilidad · Dónde estamos), no quick CTA. Transparent over the hero; from 60 px of scroll a blurred dark-blue bar, lower (GSAP). The current section's link is underlined. It hides only while the gallery passes. Phone: logo + menu button inside the header, which opens the right-hand panel with the curved edge |
...
| 4 | `FEATURE-GRID` | What the rental includes | Built 2026-09-24 (`D-037`), `id="incluye"`, a short beige band right after the gallery's four spaces. "Lo que incluye el alquiler" · "Para hasta 35 personas. La limpieza del lugar está incluida." · four numbered groups with bullets, 01 El salón (Salón de usos múltiples, Baño, Espacio de lavado con pileta) · 02 El patio (Patio exterior con pileta, 8 sillones y livings de exterior) · 03 La parrilla (Asador, Horno pizzero grande) · 04 Para la mesa (30 sillas, 3 mesas plegables, Vajilla y vasos para 30). Four columns from 900 px; one group per row on the phone. Everything visible, nothing on hover |
...
|---|---|---|
| `/admin/login` | `AUTH-CARD.signin` → split screen (`D-043`) | Built 2026-09-25. The garden photo (`public/media/fotos/jardin.jpg`) fills the left 58 % on desktop and a 38svh band on top on the phone; the right side is deep blue with TITLE "Panel de Araucaria" (Instrument Serif) · "Entrá con la contraseña del salón." · FIELD password (the public form's underlined field, "Mostrar"/"Ocultar") · SUBMIT "Entrar" (the site's `.boton`) · ERROR-STATE (below, with a drawn icon and one shake) · no "forgot password" (rotation is manual) |
| `/admin` (Inicio), `/admin/reservas`, `/admin/calendario` | `APP-SHELL.sidebar` + three sections + dialogs + confirmation alert (`D-046`) | Built 2026-09-25, deep blue like the login. **Side menu** (a bottom tab bar on the phone): "Araucaria · Panel", **"+ Nueva reserva"**, Inicio · Reservas · Calendario, and at the foot "Ver el sitio" and a **visible "Cerrar sesión"**. **Inicio:** "Próxima reserva" (the one solid beige card: day, module, client, "mañana / en N días"), reservations this month, whole free days left; "Los próximos 7 días" (each day lists its reservations as "Noche · Juan Pérez", or "Libre · reservar"); "Próximas reservas" (5, with "Cancelar"). **Reservas:** tabs "Próximas" / "Pasadas" with counts, a search by name or phone, one row per reservation (date, client, module and hours, phone that opens WhatsApp, **"Cancelar"**). **Calendario:** the month with "Hoy" and arrows; every reservation written inside its day as a beige label; "+ Reservar" on a free day; on the phone the month is an agenda that starts today |
| Nueva reserva (`D-046`, `D-047`) | Centered dialog, two columns (stacked on the phone) | **The month** (as Origin UI's Appointment picker): arrows within the bookable range, today ringed, the chosen day in beige, a **dot** under a day with a module reserved, a **red strike** on a full day, past days dimmed; legend "• Algo reservado · ~~12~~ Completo". **Beside it, the chosen day** ("Sábado 26 de septiembre") with Mediodía 10 a 17 h / Noche 19 a 02 h / Día completo 10 a 02 h, taken ones disabled and marked **"Ocupado"**. Then "Nombre de quien reserva" (required) and "Teléfono (opcional)" with a fixed **+54** · **Volver** / **Guardar reserva**. Opened from a free day of the calendar, that day comes chosen |
| Reservation detail and cancel (`D-046`) | Centered dialog, then an alertdialog | Tap a reservation → client name, the day, module and hours, phone (WhatsApp) · **Cerrar** / **Cancelar reserva**. Cancelling always asks: "¿Cancelar esta reserva?" + "Juan Pérez · Sábado 26/09 · Noche. El día vuelve a verse libre en el sitio." · **Volver** / **Sí, cancelar** |
| Confirmation alert (`D-020`) | Centered card over the dimmed page (Apple alert) | A ring and a check drawn with GSAP DrawSVG (a cross on errors), title ("Reserva guardada", "Reserva cancelada", "Deshecho"), detail "Sábado 26/09 · Noche · Juan Pérez", buttons **Deshacer** · **Listo**. Closes by itself after 4 s, paused while the pointer or keyboard focus is on it; Escape closes it |
