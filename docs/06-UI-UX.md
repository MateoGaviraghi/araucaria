...
| `/admin` | `APP-SHELL.topbar` + summary + month calendar + day panel + upcoming list + confirmation alert (`D-045`) | Built 2026-09-25, deep blue like the login. Bar: "Araucaria · Panel", "Ver el sitio" and a **visible "Cerrar sesión"**. First line: "Hoy es …". Summary: **"Lo próximo reservado"** (the one solid beige card), **"En {mes}"** (modules reserved in the month shown + whole free days left) and **"Los próximos 7 días"** (seven small split days that open the day). Month with "Hoy" and arrows; **each day split in two ("Mitades", `D-019`): top half = Mediodía, bottom half = Noche**; a reserved half is filled beige and, when the cell is wide enough, says "Mediodía"/"Noche" · legend "Libre · Reservado · Arriba mediodía · abajo noche" · **"Próximos reservados"** grouped by day with "Liberar" per module (`D-020`) |
