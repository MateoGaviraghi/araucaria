...
| OQ-10 | The first `/admin/login` load after idle takes ~3.2 s (`G-064`). Options: find why the static shell does not arrive at once (suspect: `no-store` on `/admin/*`), or a Neon plan that does not scale to zero | The owner's first impression of the panel | Mateo chose to move on (2026-09-25: *"seguir"*). Open |
...
| G-058 | The owner could not log in on production with his password: the production branch had **no `admin_credential` row**. `npm run rotate-password` writes to the database in `.env.local`, which is the dev branch, so the credential only ever existed there. Every Neon branch the app points to needs its own credential; load production's with `npm run rotate-password -- <env file with the production DATABASE_URL>` and check the printed host |
| G-059 | Playwright's `page.screenshot()` hides the caret by default by putting an inline `caret-color: transparent` on inputs. Taken before hydration, it makes React report a hydration mismatch that does not exist for real visitors. Pass `caret: "initial"` |
| G-060 | jq installed with winget writes CRLF on Windows; bash scripts that read `jq -r` output (the semgrep skill's `run-scans.sh`) then carry a `\r` into every key ("'javascript' is not a language"). A `jq` wrapper that calls `jq.exe -b` fixes it |
| G-061 | Inside a drizzle `sql` tagged template, `\+` is cooked to `+`, so a check constraint `~ '^\+54…'` reached Postgres as `'^+54…'`. Review generated SQL; use a character class (`[+]`) instead of escapes |
| G-062 | A page's inline `<script dangerouslySetInnerHTML>` runs only on a full page load. After a client navigation React does not execute it and logs "Encountered a script tag while rendering React component". The panel's logout therefore loads `/admin/login` as a full page (`window.location.replace`) |
| G-063 | `.pn button { font: inherit }` (specificity 0,1,1) silently beat every `.pn-boton { font-size }` (0,1,0): all panel buttons rendered at 16 px. Put base resets inside `:where()` |
| G-064 | Neon's free compute scales to zero after 5 idle minutes and Vercel's function also cools: the first `/admin/login` load after idle showed the form at 3.2–3.6 s, even after `D-044` moved the form to the static shell (warm: 0.3–0.5 s). The cause of the shell not arriving at once is **unconfirmed**; the `no-store` header on `/admin/*` (C-13) is the suspect (`OQ-10`) |
| G-065 | In headless checks right after a client navigation, the outgoing page's nodes are still in the DOM for a moment: a search count read 4 rows where the page had 3. Wait for the new page's own element before counting |
| G-066 | OKLab → LMS → the sRGB matrix already gives **linear** RGB. Applying the sRGB decode on top understates contrast (a pair of 8.1:1 read 4.2:1). Measure with the linear values straight into the WCAG luminance |
| G-067 | In production the header underlined "Qué incluye" (or "Dónde estamos") while "Disponibilidad" was on screen, and never "Disponibilidad". The calendar is streamed (`<Suspense>` in `app/page.tsx`), and its fallback `DisponibilidadEsperando` carries the same `id="disponibilidad"`. With the database asleep (`G-064`), the header's per-section ScrollTriggers attached to the fallback, which React then replaced with a node of another height; locally the data arrived first, so it never showed. Fix (2026-09-26): the header finds the section crossing mid-screen on every scroll update (one ScrollTrigger, `onUpdate`/`onRefresh`), and the calendar calls `ScrollTrigger.refresh()` when it arrives. Verified with a temporary 4 s delay in `Disponibilidad`: the old code failed twice, the new one 0 times at 1920 and 1440, on scroll, header clicks and wheel |
| G-068 | The footer's Instagram logo showed only its white outline on desktop. `Logo` used fixed ids (`logo-ig`, `logo-tel`, `logo-maps-pin`). Since `D-041` the phone menu also renders the Instagram logo; on desktop that panel is `display: none`, and `url(#logo-ig)` resolved to the first element with the id, inside the hidden panel, so the gradient did not paint. Fix (2026-09-26): each `Logo` instance takes its own ids from `useId()`. Checked: 0 duplicate ids on the page; the footer logo's pixels are the gradient (bottom-left 253,190,124; top-right purple) at 1920 and 375 |
